...
/**
 * Generic SOAP client for Orient Sigorta services (Policy/Utility/Security).
 * Uses fetch if available; supports optional mock mode.
 */
export class OrientSoapClient {
  ...
  private mockResponse(operation: string): SoapCallResult {
    ...
    return {
      success: true,
      raw: `<mock><operation>${operation}</operation><timestamp>${now}</timestamp></mock>`,
    };
  }
}
